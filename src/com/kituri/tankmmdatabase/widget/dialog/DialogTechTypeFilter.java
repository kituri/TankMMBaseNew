...
public class DialogTechTypeFilter extends RelativeLayout implements Selectable<Entry>, View.OnClickListener, SelectionListener<Entry>, Populatable<ListEntry>{
	...
	public DialogTechTypeFilter(Context context){
		this(context,null);
	}
	
	public DialogTechTypeFilter(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView();
	}
	...
	private void setData(ListEntry datas){
		...
		for(Entry entry : datas.getEntries()){
			entry.setViewName(ItemFilterTechType.class.getName());
			mAdapter.add(entry);
		}
	}
